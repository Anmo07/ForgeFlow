...
class InvoiceRepository:
    ...
    def create(self, db: Session, org_id: int, invoice_number: str, issue_date=None, due_date=None, status='draft', subtotal=0.0, tax_rate=0.0, tax_amount=0.0, total=0.0, notes=None, client_id=None, created_by=None, pdf_url=None) -> Invoice:
        invoice = Invoice(organization_id=org_id, invoice_number=invoice_number, issue_date=issue_date, due_date=due_date, status=status, subtotal=subtotal, tax_rate=tax_rate, tax_amount=tax_amount, total=total, notes=notes, client_id=client_id, created_by=created_by, pdf_url=pdf_url)
        db.add(invoice)
        db.commit()
        db.refresh(invoice)
        return invoice
