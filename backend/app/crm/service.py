...
CRM_METRICS_CACHE_KEY = "cache:crm:metrics:{org_id}"
CRM_METRICS_CACHE_TTL = 300  # 5 minutes
...
class CRMService:
    ...
    def _invalidate_crm_cache(self, org_id: int):
        """Invalidate the CRM metrics cache for a given organization."""
        cache_key = CRM_METRICS_CACHE_KEY.format(org_id=org_id)
    ...
    def create_client(self, db: Session, org_id: int, data: ClientCreate) -> ClientResponse:
        ...
        self._invalidate_crm_cache(org_id)
        return ClientResponse.model_validate(client)
    ...
    def get_metrics(self, db: Session, org_id: int) -> CRMMetrics:
        ...
        if cached:
            try:
                return CRMMetrics(**json.loads(cached))
            except Exception:
                logger.warning("Failed to deserialize CRM metrics cache, falling back to DB")
        ...
        metrics = CRMMetrics(active_leads=active_leads, pipeline_value=pipeline_value, deals_won_value=deals_won_value, conversion_rate=conversion_rate, total_clients=len(clients))

        # Write to cache (fail-open: if Redis is down, just skip)
        try:
            redis_client.set(cache_key, metrics.model_dump_json(), expire=CRM_METRICS_CACHE_TTL)
        except Exception:
            logger.warning("Failed to write CRM metrics to Redis cache")
